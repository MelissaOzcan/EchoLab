...
function LanguageRunner() {
    ...
    const fetchCode = async (lang) => {
        try {
            const res = await axios.get(`https://3.142.174.77:4000/editor/${id}`, {
    ...
    const updateCodeInDatabase = async (code) => {
        try {
            await axios.post(`https://3.142.174.77:4000/editor/${id}`, { code, lang: language }, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        const compileUrl = `https://3.142.174.77:4000/compile/${language.toLowerCase()}`;
